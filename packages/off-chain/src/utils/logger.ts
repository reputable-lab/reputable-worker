...
let logtail;
if (process.env.LOGTAIL_ACCESS_TOKEN_OFF_CHAIN) {
  logtail = new Logtail(process.env.LOGTAIL_ACCESS_TOKEN_OFF_CHAIN);
} else {
  console.log('Logtail backup is off. Be sure to be in testing!');
}
...
if (logtail) {
  transportArray.push(new LogtailTransport(logtail) as any);
}

const logger = createLogger({
  defaultMeta: { service: 'Off-Chain' },
  level: 'info',
